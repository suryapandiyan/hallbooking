...
router.post("/createuser", async (req, res) => {
  ...
  const user = await customer_ids.find(
    (userid) => userid.customer_name == customer_name
  );
  try {
...
router.post("/:username/bookrooms/:hall_id", async (req, res) => {
  ...
  if (new Date(start_time) > new Date(end_time))
    return res.send("start time must be before end time");
  else if (new Date(start_time) < new Date())
    return res.send("start time must be after current time");
  ...
  try {
    ...
    const availability = await booking_ids.some((booking) => {
      ...
      if (booking.hall_id == hall_id) {
        if (
          (new Date(booking.booked_at[0]) <= new Date(start_time) &&
            new Date(start_time) < new Date(booking.booked_at[1])) ||
          (new Date(booking.booked_at[0]) < new Date(end_time) &&
            new Date(end_time) <= new Date(booking.booked_at[1]))
        ) {
    ...
    if (!availability) {
      ...
      return customer_ids.map((customer) => {
        if (customer.customer_id == customer_id)
          customer.booked_id = booking_id;
      });
    }
  } catch (err) {
    console.log(err);
...
router.delete("/:username/cancelroom/:booking_id", async (req, res) => {
  ...
  const booking = await booking_ids.find(
    (booked) => booked.booking_id == booking_id
  );
  if (user && booking) {
...
router.get("/:username/mybooking", async (req, res) => {
  ...
  console.log(booking_ids);
  if (!user) return res.status(400).send("user not exist");
  const booking_list = await booking_ids.filter(
    (userid) => userid.customer_id == user.customer_id
  );
...
router.get("/:username/mybooking_history", async (req, res) => {
  ...
  if (!user) return res.status(400).send("user not exist");
  const booking_list = await booking_history.filter(
    (userid) => userid.customer_id == user.customer_id
  );
  return res.status(200).json(booking_list);
});
