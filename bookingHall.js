...
setInterval(() => {
  ...
  checkout.forEach((hall) => {
    const customer = booking_history.find(
      (customer) => customer.customer_id == hall.customer_id
    );
    if (customer)
      booking_history.forEach((history) => {
  ...
  hall_ids.forEach((hall) => {
    booking_ids.forEach((booking) => {
      if (booking.hall_id == hall.hall_id) {
        new Date(booking.booked_at[0]) <= new Date() &&
        new Date() < new Date(booking.booked_at[1])
          ? (hall.available_now = false)
          : (hall.available_now = true);
      }
    });
  });
}, 60000 * 30);
